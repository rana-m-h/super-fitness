...
export const getMealsCategories = async () => {
  ...
  const payload: APIResponse<PaginatedResponse<{ categories: Meals[] }>> = await response.json();
...
export const getMealDetails = async (id: string) => {
  ...
  const payload: APIResponse<PaginatedResponse<{ meals: MealDetails[] }>> = await response.json();
